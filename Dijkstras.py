...
DESCRIPTION:        Contains code for Dijstras algorithm used to find the shortest path between two nodes on a graph

                    Original author is DAVID EPPSTEIN
...
def Dijkstra(graph,start,end=None):
    ...
    A vertex can be any object that can
...
def DijkstraShortestPath(graph,start,end):
    ...
    while 1:
        ...
        if end == start: break
        #print "End: " + str(end)
        #print "Predec: " + str(predecessors)
        end = predecessors[end]
    path.reverse()
    return path

#print shortestPath(r.STATIONS_DISTANCES, 'BARKING', 'MILE END')
